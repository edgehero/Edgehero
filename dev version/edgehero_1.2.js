...
(function (window, $) {
    ...
    EdgeHero.performHeroics = function () {
		...
		///// scroll horizontal
		        $("[class^='hscroll_'],[class*=' hscroll_']").each(function () {
            var a = $(this).attr("class");
            var b = '#Stage_' + a.split("scroll_")[1].split(" ")[0]
            var c = b
            var mystage = $(this).attr('style') + 'display:block;'
            ...
            $('a[href^="#"]').click(function (event) {
                var id = $(this).attr("href");
                var offset = 10;
                var target = $(id).offset().left - offset;
                $('html, body').animate({
                    scrollLeft: target
                }, 500);
                event.preventDefault();
		...
		////////// Background image
		        ...
		        $(".backgroundimage").each(function () {
					...
					if($.trim($(".backgroundimage").attr("src")) != "") {
     				 image = $(this).attr('src');
					...
					$(this).replaceWith('<div style="' + mystyle + '" id="' + $(this).attr('id') + '" class="' + $(this).attr('class') + '" >'+ $(this).html() + '</div>' );
			$(this).css({
        ...
        $(".form").each(function () {
            ...
            $(this).replaceWith('<form id="' + $(this).attr('id') + '"method="post" class="' + $(this).attr('class') + '" style="' + thisformstyle + '">' + $(this).html() + '</form>')
        ...
        $(".input-submit").each(function () {
            var thisinputstyle = $(this).attr('style');
            $(this).replaceWith('<input type="submit" id="' + $(this).attr('id') + '" class="' + $(this).attr('class') + '" style="' + thisinputstyle + '">' + $(this).html() + '</input>')
        });
        $(".span").each(function () {
		...
		$("[class^='val_'],[class*=' val_']").each(function () {
            var myname = $(this).attr("class");
            var mynamegoal = myname.split("val_")[1].split(" ")[0]
            $(this).attr("value", mynamegoal);
        });
        ...
        $("[class^='videolink'],[class*=' videolink']").each(function () {
			...
			if($(this).is("video")){
				console.log(a + " = registered as already being a video");
				...
				$(myd).empty(); // empty class first, before replacing it with video
        ...
        $("[class^='backgroundvideo'],[class*=' backgroundvideo']").each(function () {
			var a = $(this).attr("class");
			if($(this).is("video")){ // if it is already a video from edge animate, than just add the correct css to the video
				console.log(a + " = registered as already being a video");
					$(this).css({
						"display": "block",
						"position": "absolute",
            ...
            $(this).replaceWith('<video height="auto" id="' + $(this).attr('id') + '" class="' + a + '" style="display:block;position: absolute;bottom: 0px; right: 0px; min-width: 100%; min-height: 100%; width: auto; height: auto; overflow: hidden;" webkit-playsinline>' + $(this).html() + '</video>')
